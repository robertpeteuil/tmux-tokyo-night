...
function generate_left_side_string() {
  ...
  host_icon=$(get_tmux_option "@theme_host_icon" " ")
	...
	echo "#[bg=${PALLETE['blue']},fg=${PALLETE['bg_dark']}] ${host_icon}#H #[fg=${PALLETE[fg_gutter]},bold]#{?client_prefix,#[bg=${PALLETE[yellow]}],#[bg=${PALLETE[green]}]}${session_icon}#S ${separator_end}"
}
